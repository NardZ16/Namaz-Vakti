...
const MenuModal: React.FC<MenuModalProps> = ({ isOpen, onClose, onSelectTool }) => {
  ...
  return (
    <div className={`fixed inset-0 z-[120] flex flex-col justify-end items-center transition-all duration-300 ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
      ...
      <div 
        className={`w-full max-w-md bg-white dark:bg-[#1e293b] rounded-t-[2.5rem] p-6 shadow-[0_-20px_50px_rgba(0,0,0,0.1)] transition-transform duration-500 ease-out border-t border-amber-200/50 dark:border-slate-700 relative overflow-hidden mb-[140px] mx-4 ${isOpen ? 'translate-y-0' : 'translate-y-full'}`}
      >
