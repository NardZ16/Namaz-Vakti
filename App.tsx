...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f5f2eb] dark:bg-[#0c1218] text-gray-800 dark:text-gray-100 font-sans transition-colors duration-300 selection:bg-teal-500 selection:text-white relative overflow-hidden flex flex-col">
      ...
      {toastMessage && (
        <div className="fixed bottom-48 left-1/2 -translate-x-1/2 bg-gray-800 dark:bg-slate-200 text-white dark:text-slate-900 px-6 py-3 rounded-full shadow-xl text-sm font-bold z-[120] animate-in slide-in-from-bottom-2 fade-in duration-300 flex items-center gap-2">
      ...
      {/* Main Scroll Content - Increased bottom padding to accommodate raised navbar */}
      <main className={`flex-1 relative z-10 px-4 max-w-screen-xl mx-auto flex flex-col items-center gap-4 overflow-y-auto pb-[220px] ${activeTool ? 'hidden' : 'pt-[74px]'}`}>
      ...
      {/* Floating Bottom Navigation - RAISED to bottom-[90px] to avoid ad overlap */}
      <div className="fixed left-0 right-0 z-[130] flex justify-center pointer-events-none transition-all duration-300 bottom-[90px]">
      ...
      {/* STICKY BOTTOM AD AREA - Metin kaldırıldı, zemin bırakıldı */}
      <div className="fixed bottom-0 left-0 right-0 h-[65px] bg-white dark:bg-[#0c1218] border-t border-gray-200 dark:border-slate-800 flex items-center justify-center z-[90] pointer-events-auto select-none shadow-[0_-4px_10px_rgba(0,0,0,0.05)]">
      ...
      {/* Menu Modal - Z-index [120] which is BELOW the Navbar [130] - Increased mb for lifted navbar */}
      <MenuModal 
        isOpen={isMenuOpen} 
      ...
      {/* Tool Container - Content Layer - Adjusted padding for raised navbar */}
      {activeTool && (
         <div className="fixed inset-0 z-[70] bg-[#f5f2eb] dark:bg-[#0c1218] flex flex-col animate-in slide-in-from-right duration-300 pt-[env(safe-area-inset-top)] pb-[180px]">
            {activeTool !== 'quran' && (
